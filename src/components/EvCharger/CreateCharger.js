  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (updateDispatch) {
        ...
        dispatch(createChargerUpdateAction(updateValues));
      } else {
        dispatch(createChargerAction(values));
      }
      closeModal();
    },
    ...
    >
      <div className="relative">
        <button className="absolute top-6 right-6 p-2 text-red-800 hover:text-sky-950">
          <XCircleIcon className="h-12 w-12" onClick={closeModal} />
        </button>
      </div>
            ...
            <form onSubmit={formik.handleSubmit} className="space-y-6">
              <div className={"bg-sky-300 p-3 rounded"}>
                ...
                <Alert severity="info" className={"mt-2"}>
                  If your charger is not on the map, zoom out and select the
                  closest one.
                </Alert>
