...
import {
  deleteChargerAction,
  resetCreateAction,
} from "../../redux/slices/evCharger/chargerSlices";
...
const CreatePost = () => {
  ...
  const { loading, appErrPost, serverErrPost, isCreated } = post;
  ...
  //add and delete charger from array
  useEffect(() => {
    if (chargerCreated) {
      addCharger((prevChargers) => {
        if (
          prevChargers.some((charger) => charger._id === chargerCreated._id)
        ) {
          return prevChargers;
        }
        return [...prevChargers, chargerCreated];
      });
    }
    if (chargerDeleted) {
      addCharger((prevChargers) =>
        prevChargers.filter((charger) => charger._id !== chargerDeleted?._id)
      );
    }
    if (chargerUpdated) {
      addCharger((prevChargers) =>
        prevChargers?.map((charger) =>
          charger._id === chargerUpdated._id ? chargerUpdated : charger
        )
      );
    }
  }, [chargerCreated, chargerDeleted, addCharger, chargerUpdated]);
  ...
  useEffect(() => {
    if (serverErrPost || appErrPost || appErrCharger || serverErrCharger) {
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
  }, [serverErrPost, appErrPost, appErrCharger, serverErrCharger]);
        ...
        <div className="sm:mx-auto sm:w-full sm:max-w-2xl">
          ...
          {appErrPost || serverErrPost ? (
            <div
              ...
              role="alert"
            >
              <strong className="font-bold">Post Error! </strong>
              <span className="block sm:inline">
                {serverErrPost}! {appErrPost}
              </span>
            </div>
          ) : null}
          {appErrCharger || serverErrCharger ? (
            <div
              className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 my-1 rounded relative"
              role="alert"
            >
              <strong className="font-bold">Charger Error! </strong>
              <span className="block sm:inline">
            ...
            <form onSubmit={formik.handleSubmit} className="space-y-6">
              ...
              <div>
                      ...
                      <TableBody>
                        {chargers?.map((charger, i) => (
                          <TableRow key={i} className="border-b">
                            ...
                            <TableCell>
                              <span className="text-sm">{charger?.title}</span>
                            </TableCell>
                            <TableCell>
                ...
                <button
                  ...
                  className="flex items-center bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg mt-2"
                >
                  ...
                  {loading ? (
                    <>
                      <Alert severity="info" className={"my-2"}>
                        We are in the process of creating your post and
                        calculating the recommended route. Please bear with us
                        while we complete these tasks. Thank you for your
                        patience.
                      </Alert>
                      <button
                        disabled
